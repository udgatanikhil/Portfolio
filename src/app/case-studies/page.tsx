      ...
      <section className="space-y-20">
        {caseStudies.map((project, idx) => (
          <article
            key={project.slug}
            id={project.slug}
            className="scroll-mt-20 border-t border-slate-200 dark:border-slate-800 pt-16 first:border-t-0 first:pt-0"
          >
            <div className="space-y-8">
              {/* Header Info */}
              <div className="space-y-2">
                <span className="text-xs font-mono uppercase tracking-wider text-indigo-650 dark:text-indigo-400 font-semibold">
              ...
              {/* Meta Stats Panel */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800">
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-4">
                ...
                <div className="md:col-span-8 space-y-8">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-mono tracking-widest uppercase text-slate-400 dark:text-slate-500 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-rose-500/85"></span>
                      Problem Statement
                    </h4>
                    ...
                    <ul className="space-y-2.5 pl-1">
                      {project.architecture.map((item, idx) => (
                        <li key={idx} className="flex gap-3 text-sm text-slate-650 dark:text-slate-350 leading-relaxed font-light">
                          <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold text-xs mt-0.5">
                            [{idx + 1}]
                          </span>
                          <span>{item}</span>
                        </li>
                  ...
                  {/* Project Outcome */}
                  <div className="border-l-2 border-emerald-555 pl-5 space-y-2 bg-emerald-50/10 dark:bg-emerald-950/5 py-4 pr-4 rounded-r-2xl">
                    <h4 className="text-xs font-mono tracking-widest uppercase text-emerald-700 dark:text-emerald-400 flex items-center gap-2">
                ...
                <div className="md:col-span-4 space-y-4">
                  <h4 className="text-xs font-mono tracking-widest uppercase text-slate-400 dark:text-slate-500 flex items-center gap-2 border-b border-slate-205 dark:border-slate-800 pb-2.5">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {project.kpis.map((kpi, kpiIdx) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400 mt-1.5">
                          {kpi.metric}
                        </span>
                      </div>
                    ))}
