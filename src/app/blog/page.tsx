    ...
    <div className="space-y-12 max-w-3xl py-8 md:py-16 animate-fade-up">
      ...
      <section className="space-y-10">
        {blogPosts.map((post) => (
          <article
            key={post.slug}
            className="group relative flex flex-col items-start border-b border-slate-900 pb-8 last:border-b-0 last:pb-0"
          >
            ...
            <h3 className="text-lg font-bold tracking-tight text-white group-hover:text-cyan-400 transition-colors duration-250">
              <Link href={`/blog/posts/${post.slug}`}>
                {post.title}
              </Link>
            </h3>

            <p className="text-xs text-slate-450 mt-2 leading-relaxed font-light">
              {post.excerpt}
            </p>
